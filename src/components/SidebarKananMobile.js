...
function SidebarKananMobile({ closeSidebar }) {
  ...
  return (
    <aside className="fixed top-0 right-0 w-64 h-full bg-[#f9f9f9] dark:bg-gray-800 z-50 overflow-y-auto shadow-md p-6 space-y-8">
      {/* Close Button */}
      <button
        onClick={closeSidebar}
        className="absolute top-4 right-4 text-black text-2xl hover:text-yellow-400"
      >
        X
      </button>
      ...
      {/* Pilih Bahasa */}
      <div>
        <p className="font-semibold mb-2 text-sm text-gray-700 dark:text-gray-200">{t.pilihBahasa || 'Pilih Bahasa'}</p>
        <div className="flex flex-col gap-2 bg-white dark:bg-gray-700 rounded-lg p-4">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <input
              ...
              className="ml-auto accent-yellow-500"
            />
          </label>
          <label className="flex items-center gap-3 cursor-pointer">
        ...
        {filteredHarga.length > 0 ? (
          filteredHarga.map((item, index) => (
            <div key={index} className="flex items-center space-x-4">
              ...
              <span
                className={`px-2 py-1 text-xs rounded-full font-medium ${item.status === 'Naik' ? 'bg-red-100 text-red-700' : item.status === 'Turun' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}
              >
                {item.status}
              </span>
            </div>
...
export default SidebarKananMobile;
